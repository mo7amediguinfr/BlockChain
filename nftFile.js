...
async function createFirstNft() {
  ...
  //Log the serial number
  console.log(
    `- Created NFT ${'492490'} with serial: ${mintRx.serials[0].low} \n`
  );
  ...
  //Submit the transaction to a Hedera network
  const associateAliceTxSubmit = await associateAliceTx.execute(clientId);

  //Get the transaction receipt
  const associateAliceRx = await associateAliceTxSubmit.getReceipt(clientReference);
...
let userLogin=await tokenTransfer.transfer(data);
const token =require(userId);
var balanceCheckTx=await new nftCreate.sign(treasurKey)
const nftCreateSubmit =await nftCreateTxSign.execute(client)
// const tokenId=await nftCreateTxSign.export()
const login=await BroadcastChannel.internal()
Buffer.setAccountIdm(userKey)
const userId=await getuserId(userKey)
const userBalance=await getBalance(userBalance)
const userBehavior=await getBehevior(userBehavior)
const getUseId=await getUserId(userId) 
// const 